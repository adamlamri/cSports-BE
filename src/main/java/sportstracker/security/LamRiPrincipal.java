...
import sportstracker.config.SecurityConfig;
...
public class LamRiPrincipal implements OidcUser, UserDetails, CredentialsContainer {
    ...
    public Collection<? extends GrantedAuthority> getAuthorities() {
        ...
        Collection<SpectreGrantedAuthority> authorities = roles.stream()
                .map(role -> new SpectreGrantedAuthority("ROLE_" + role))
                .collect(Collectors.toCollection(() ->
                        new ArrayList<>(roles.size() + 2)));

        if (userDto.isGoodUser()) {
            authorities.add(new SpectreGrantedAuthority("ROLE_"
